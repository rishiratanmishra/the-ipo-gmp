...
class IPOD_Fetcher
{
    ...
    public static function fetch_all()
    {
        ...
        $sql = $wpdb->prepare("
            ...
            WHERE
                ...
                OR (m.status IN ('close', 'closed', 'allotment') AND STR_TO_DATE(m.listing_date, '%%b %%d, %%Y') >= CURDATE() AND d.fetched_at < %s)
            ORDER BY 
                (d.fetched_at IS NULL) DESC,
                FIELD(UPPER(m.status),'OPEN','ALLOTMENT','CLOSE','CLOSED','UPCOMING'),
                m.id DESC
            LIMIT %d
        ", $cutoff_hour, $cutoff_day, $cutoff_hour, $limit);
    ...
    private static function should_fetch($ipo)
    {
        ...
        if ($status === 'OPEN')
            return ($now - $last) > 3600; // 1 Hour
        if ($status === 'UPCOMING')
            return ($now - $last) > 86400; // 24 Hours
        if ($status === 'CLOSE' || $status === 'CLOSED' || $status === 'ALLOTMENT')
            return ($now - $last) > 3600; // 1 Hour
    ...
    public static function scrape_data($id, $slug)
    {
        ...
        if ($aboutHeader) {
            ...
            $h = $aboutHeader;
            while ($h && !preg_match('/^h[1-6]$/i', $h->nodeName))
                $h = $h->parentNode;
            $h = $h ?: $aboutHeader;
        ...
        if (stripos(trim($data['about_company'] ?? ''), $soon) === 0 && strlen($data['about_company']) < 50)
            $data['about_company'] = '';
        ...
        if ($strengthNode) {
            foreach ($xpath->query(".//li", $strengthNode) as $li)
                $strengths[] = $clean($li->textContent);
            if (empty($strengths)) {
                $stext = $clean($strengthNode->textContent);
                if (stripos($stext, $soon) === false || strlen($stext) > 50)
                    $data['strengths_text'] = $stext;
            }
        }
        ...
        if ($weaknessNode) {
            foreach ($xpath->query(".//li", $weaknessNode) as $li)
                $weaknesses[] = $clean($li->textContent);
            if (empty($weaknesses)) {
                $wtext = $clean($weaknessNode->textContent);
                if (stripos($wtext, $soon) === false || strlen($wtext) > 50)
                    $data['weaknesses_text'] = $wtext;
            }
        }
