 ...
 * Description: Dedicated page for all SME IPOs (History & Active).
 */

global $wpdb;
$t_master = $wpdb->prefix . 'ipomaster';

// Fetch All SME IPOs (Paginated in real app, but fetching all for now)
...
<main class="max-w-[1280px] mx-auto px-10 py-8">
    ...
    <div class="mb-10 pt-4">
        <h1 class="text-white text-4xl font-black mb-2 tracking-tight">SME IPOs <span class="text-purple-accent">Dashboard</span></h1>
        <p class="text-slate-400 text-lg max-w-2xl font-medium">Tracking the high-growth Small & Medium Enterprise segment.</p>
    </div>
    ...
    <div class="overflow-hidden rounded-xl border border-border-navy bg-[#0B1220] shadow-2xl">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-border-navy">
                ...
                <tr class="data-table-row transition-colors cursor-pointer group" onclick="window.location.href='<?php echo esc_url($details_url); ?>'">
                    <td class="px-6 py-5">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <p class="text-base font-bold text-white group-hover:text-purple-accent transition-colors"><?php echo esc_html($ipo->name); ?></p>
                ...
                <?php endforeach; else: ?>
                <tr><td colspan="5" class="px-6 py-8 text-center text-slate-500">No SME IPOs found in database.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
