...
<head>
    ...
    <style>
        ...
        .tab-btn.active {
            background: linear-gradient(135deg, #0d7ff2 0%, #0a5bb5 100%);
            color: white;
        }
...
<main class="max-w-[1400px] mx-auto px-4 md:px-10 py-6">
    ...
    <div class="glass-card border border-border-navy rounded-[24px] p-6 mb-6 sticky top-0 z-40">
        <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
            ...
            <div class="flex items-center gap-4">
                <div class="size-16 rounded-xl bg-white p-2 flex items-center justify-center shadow-xl ring-2 ring-white/10 overflow-hidden shrink-0">
                ...
                <div>
                    ...
                    <div class="flex flex-wrap gap-2 mt-2">
                        <span class="px-2.5 py-1 bg-[#1E293B] text-slate-300 text-[9px] font-black uppercase tracking-widest rounded border border-slate-700">
                            <?php echo esc_html($ipo->status); ?>
                        </span>
                        <span class="px-2.5 py-1 bg-[#0F3864] text-blue-300 text-[9px] font-black uppercase tracking-widest rounded border border-blue-500/30">
    ...
    <div class="glass-card border border-border-navy rounded-[20px] mb-6 overflow-hidden">
        <div class="overflow-x-auto custom-scrollbar">
            <div class="flex gap-2 p-4 min-w-max">
                <button onclick="switchTab('overview')" class="tab-btn active px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all" data-tab="overview">
                    <span class="material-symbols-outlined text-sm mr-2 align-middle">dashboard</span>Overview
                </button>
                <button onclick="switchTab('subscriptions')" class="tab-btn px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all bg-slate-900/40 text-slate-400 hover:bg-slate-800" data-tab="subscriptions">
                    <span class="material-symbols-outlined text-sm mr-2 align-middle">leaderboard</span>Subscriptions
                </button>
                <button onclick="switchTab('financials')" class="tab-btn px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all bg-slate-900/40 text-slate-400 hover:bg-slate-800" data-tab="financials">
                    <span class="material-symbols-outlined text-sm mr-2 align-middle">payments</span>Financials
                </button>
                <button onclick="switchTab('analysis')" class="tab-btn px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all bg-slate-900/40 text-slate-400 hover:bg-slate-800" data-tab="analysis">
                    <span class="material-symbols-outlined text-sm mr-2 align-middle">analytics</span>Analysis
                </button>
                <button onclick="switchTab('reviews')" class="tab-btn px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all bg-slate-900/40 text-slate-400 hover:bg-slate-800" data-tab="reviews">
                    <span class="material-symbols-outlined text-sm mr-2 align-middle">rate_review</span>Reviews
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div class="lg:col-span-3">
            ...
            <div id="tab-overview" class="tab-content fade-in space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    <div class="glass-card border border-border-navy rounded-xl p-5 hover:border-primary/40 transition-all">
                        <span class="text-[10px] uppercase tracking-widest font-black text-slate-500 block mb-2">Open Date</span>
                        <span class="text-base font-black text-white"><?php echo $ipo->open_date ? date('M j, Y', strtotime($ipo->open_date)) : 'TBA'; ?></span>
                    </div>
                    <div class="glass-card border border-border-navy rounded-xl p-5 hover:border-primary/40 transition-all">
                        <span class="text-[10px] uppercase tracking-widest font-black text-slate-500 block mb-2">Close Date</span>
                        <span class="text-base font-black text-white"><?php echo $ipo->close_date ? date('M j, Y', strtotime($ipo->close_date)) : 'TBA'; ?></span>
                    </div>
                    <div class="glass-card border border-border-navy rounded-xl p-5 hover:border-primary/40 transition-all">
                        <span class="text-[10px] uppercase tracking-widest font-black text-slate-500 block mb-2">IPO Size</span>
                        <span class="text-base font-black text-white">₹<?php echo esc_html($ipo->issue_size_cr ?: 'TBA'); ?> cr</span>
                    </div>
                    <div class="glass-card border border-border-navy rounded-xl p-5 hover:border-primary/40 transition-all">
                        <span class="text-[10px] uppercase tracking-widest font-black text-slate-500 block mb-2">Lot Size</span>
                        <span class="text-base font-black text-white"><?php echo esc_html($ipo->lot_size ?: 'TBA'); ?></span>
                    </div>
                    <div class="glass-card border border-border-navy rounded-xl p-5 hover:border-primary/40 transition-all">
                        <span class="text-[10px] uppercase tracking-widest font-black text-slate-500 block mb-2">Allotment</span>
                        <span class="text-base font-black text-white"><?php echo $ipo->allotment_date ? date('M j, Y', strtotime($ipo->allotment_date)) : 'TBA'; ?></span>
                    </div>
                    <div class="glass-card border border-border-navy rounded-xl p-5 hover:border-primary/40 transition-all">
                        <span class="text-[10px] uppercase tracking-widest font-black text-slate-500 block mb-2">Listing</span>
                ...
                <div class="glass-card border border-border-navy rounded-2xl p-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div class="flex justify-between items-center p-4 rounded-xl bg-slate-900/30 border border-white/5">
                ...
                <div class="glass-card border border-border-navy rounded-2xl p-6">
                    ...
                    <div class="text-slate-400 leading-relaxed text-xs font-medium prose prose-invert max-w-none">
                        <?php echo nl2br(esc_html($details['about_company'])); ?>
                    </div>
                    <?php if(!empty($details['address'])): ?>
                    <div class="mt-6 pt-6 border-t border-border-navy">
                        <h3 class="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Registered Address</h3>
                        <p class="text-xs text-slate-300 italic"><?php echo esc_html($details['address']); ?></p>
                    </div>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
                ...
                <div class="glass-card border border-border-navy rounded-2xl p-6">
                    ...
                    <div class="flex flex-wrap gap-3">
                        ...
                        <div class="flex items-center gap-2 bg-slate-900/50 border border-white/5 px-4 py-2.5 rounded-xl">
                            <span class="size-2 rounded-full bg-primary"></span>
                            <span class="text-xs font-bold text-slate-200"><?php echo esc_html($lm['name']); ?></span>
                        </div>
                        <?php endforeach; ?>
                    </div>
                </div>
            ...
            <div id="tab-subscriptions" class="tab-content hidden space-y-6">
                ...
                <div class="glass-card border border-border-navy rounded-2xl p-6">
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <span class="bg-neon-emerald text-[#050A18] text-[9px] font-black px-3 py-1 rounded uppercase tracking-wider shadow-[0_0_10px_rgba(0,255,148,0.3)]">Live</span>
                    </div>
                    <div class="overflow-x-auto custom-scrollbar">
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-border-navy">
                                <?php if(isset($details['subscription']) && !empty($details['subscription'])): foreach($details['subscription'] as $s): ?>
                                <tr class="text-xs font-bold data-table-row">
                                    <td class="py-3 px-2 text-slate-300"><?php echo esc_html($s['Category'] ?? $s['Investor Category'] ?? '-'); ?></td>
                                    ...
                                    <td class="py-3 px-2 text-right text-neon-emerald"><?php echo esc_html($s['Times'] ?? $s['Times Subscribed'] ?? '0.00'); ?>x</td>
                                </tr>
                                <?php endforeach; else: ?>
                ...
                <?php if(!empty($details['subscription_demand']) || !empty($details['qib_interest'])): ?>
                            ...
                            <table class="w-full text-left">
                                ...
                                <tbody class="divide-y divide-border-navy">
                                    ...
                                    <tr class="font-bold data-table-row">
                                        ...
                                        <td class="py-3 px-2 text-slate-300 text-xs"><?php echo esc_html($row[$h] ?: '-'); ?></td>
                                        <?php endforeach; ?>
                                    </tr>
                                    <?php endforeach; ?>
                                </tbody>
                    ...
                    <div class="glass-card border border-border-navy rounded-2xl p-6">
                        <h2 class="text-base font-bold flex items-center gap-2 mb-6">
                            <span class="material-symbols-outlined text-neon-emerald text-lg">account_balance</span>
                            QIB Interest (Anchor Investors)
                        </h2>
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                            <?php foreach($details['qib_interest'] as $qib): ?>
                            <div class="flex items-center gap-3 p-3 bg-slate-900/50 border border-white/5 rounded-xl hover:border-neon-emerald/30 transition-all">
                                ...
                                <span class="text-xs font-bold text-slate-300 leading-tight"><?php echo esc_html($qib); ?></span>
                            </div>
                            <?php endforeach; ?>
                        </div>
                    </div>
                    ...
                    <div class="overflow-x-auto custom-scrollbar">
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-border-navy">
                                ...
                                <tr class="text-xs font-bold data-table-row">
                                    <td class="py-3 px-2 text-slate-300"><?php echo esc_html($ab['Category'] ?? '-'); ?></td>
                                    <td class="py-3 px-2 text-center text-slate-200"><?php echo esc_html($ab['Reserved'] ?? '-'); ?></td>
                                    <td class="py-3 px-2 text-center text-slate-500 font-medium"><?php echo esc_html($ab['Applied'] ?? '-'); ?></td>
                                    <td class="py-3 px-2 text-right text-primary"><?php echo esc_html($ab['Times'] ?? '-'); ?></td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
                        ...
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-border-navy">
                                ...
                                <tr class="text-xs font-bold data-table-row">
                                    ...
                                    <td class="py-3 px-2 text-center text-slate-500"><?php echo esc_html($lot['Qty'] ?? $lot['Shares'] ?? '-'); ?></td>
            ...
            <div id="tab-financials" class="tab-content hidden space-y-6">
                ...
                <div class="glass-card border border-border-navy rounded-2xl p-6">
                    <h2 class="text-base font-bold flex items-center gap-2 mb-6">
                        <span class="material-symbols-outlined text-purple-accent text-lg">account_balance_wallet</span>
                        Company Financials
                    </h2>
                    <div class="overflow-x-auto custom-scrollbar">
                        <table class="w-full text-left">
                ...
                <div class="glass-card border border-border-navy rounded-2xl p-6">
                    <h2 class="text-base font-bold flex items-center gap-2 mb-6">
                        <span class="material-symbols-outlined text-neon-emerald text-lg">analytics</span>
                ...
                <div class="space-y-6">
                    ...
                    <div class="glass-card border border-border-navy rounded-2xl p-6">
                        <h2 class="text-base font-bold flex items-center gap-2 mb-6">
                            <span class="material-symbols-outlined text-purple-accent text-lg">payments</span>
            ...
            <div id="tab-analysis" class="tab-content hidden space-y-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <!-- Strengths -->
                    <div class="glass-card border border-border-navy rounded-2xl p-6">
                        <h2 class="text-lg font-black flex items-center gap-2 mb-6 pb-3 border-b border-neon-emerald/20">
                        ...
                        <ul class="space-y-4">
                            ...
                            <li class="flex gap-4 group">
                                <div class="flex items-start justify-center size-7 mt-0.5 rounded-lg bg-neon-emerald/10 shrink-0 font-black text-neon-emerald text-xs border border-neon-emerald/20 group-hover:bg-neon-emerald/20 transition-all">
                                    <?php echo ($index + 1); ?>
                                </div>
                    ...
                    <div class="glass-card border border-border-navy rounded-2xl p-6">
                        ...
                        <ul class="space-y-4">
                            ...
                            <li class="flex gap-4 group">
                                <div class="flex items-start justify-center size-7 mt-0.5 rounded-lg bg-red-400/10 shrink-0 font-black text-red-400 text-xs border border-red-400/20 group-hover:bg-red-400/20 transition-all">
                                    <?php echo ($index + 1); ?>
                                </div>
                                <p class="text-sm font-medium text-slate-300 leading-relaxed">
                                    <?php echo esc_html($w); ?>
        ...
        <div class="space-y-6">
                ...
                <div class="bg-slate-900/40 border border-white/5 p-4 rounded-xl flex flex-col gap-4 group hover:border-primary/20 transition-all">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex items-center gap-2 text-[10px] text-slate-400">
                             ...
                             <a href="mailto:<?php echo esc_attr($details['registrar_email']); ?>" class="hover:text-primary transition-colors"><?php echo esc_html($details['registrar_email']); ?></a>
                        </div>
                        <?php endif; ?>
                    </div>
                    <?php if(!empty($details['registrar_url'])): ?>
                    <a href="<?php echo esc_url($details['registrar_url']); ?>" target="_blank" class="w-full py-2.5 bg-[#1E293B] hover:bg-primary text-white text-[10px] font-black text-center uppercase tracking-[0.1em] rounded-lg transition-all shadow-md">Check Allotment Status</a>
                    <?php else: ?>
            ...
            <!-- Documents -->
            <?php if(isset($details['documents']) && !empty($details['documents'])): ?>
            <div class="glass-card border border-border-navy rounded-2xl p-6">
