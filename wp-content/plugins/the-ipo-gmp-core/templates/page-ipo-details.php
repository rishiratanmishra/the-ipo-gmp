...
// ... (Search Logic Removed - Handled by Theme/Header) ...
// The original search logic was also outputting a full HTML page. 
// We should rely on standard WP Search or a specific search template. 
// For now, if we want to keep the custom search results view, we should move it to search.php in the theme.
// However, to keep this refactor simple, we will assume standard interactions.
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-6 w-full lg:w-auto">
                ...
                <div class="text-center lg:text-left lg:border-l border-slate-700/50 lg:pl-6">
                    ...
                    <p class="text-lg font-black text-white leading-none">
                        <?php echo esc_html($ipo->price_band ?: 'TBA'); ?></p>
                </div>
                <div class="text-center lg:text-left lg:border-l border-slate-700/50 lg:pl-6">
        ...
        <div class="overflow-x-auto custom-scrollbar">
            <div class="flex gap-2 p-4 min-w-max">
                <button onclick="switchTab('overview')"
                    class="tab-btn active px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all"
                    data-tab="overview">
                ...
                <button onclick="switchTab('subscriptions')"
                    class="tab-btn px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all bg-slate-900/40 text-slate-400 hover:bg-slate-800"
                                    ...
                                    foreach ($details['subscription'] as $s): ?>
                                        <tr class="text-xs font-bold data-table-row">
                                            <td class="py-3 px-2 text-slate-300">
                                                <?php echo esc_html($s['Category'] ?? $s['Investor Category'] ?? '-'); ?></td>
                                            <td class="py-3 px-2 text-center text-slate-500 font-medium">
                                                <?php echo esc_html($s['Offered'] ?: '-'); ?></td>
                                            <td class="py-3 px-2 text-center text-slate-500 font-medium">
                                                <?php echo esc_html($s['Applied'] ?: '-'); ?></td>
                                            <td class="py-3 px-2 text-right text-neon-emerald">
                                                <?php echo esc_html($s['Times'] ?? $s['Times Subscribed'] ?? '0.00'); ?>x</td>
                                        </tr>
                                    <?php endforeach; else: ?>
                                    <tr>
                                        ...
                                        <?php foreach ($details['subscription_demand'] as $row): ?>
                                            <tr class="font-bold data-table-row">
                                                <?php foreach ($headers as $h): ?>
                                                    <td class="py-3 px-2 text-slate-300 text-xs">
                                                        <?php echo esc_html($row[$h] ?: '-'); ?></td>
                                                <?php endforeach; ?>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                <?php foreach ($details['qib_interest'] as $qib): ?>
                                    <div
                                        ...
                                        <span
                                            class="text-xs font-bold text-slate-300 leading-tight"><?php echo esc_html($qib); ?></span>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                        </div>
                                    ...
                                    <?php foreach ($details['application_breakup'] as $ab): ?>
                                        <tr class="text-xs font-bold data-table-row">
                                            ...
                                            <td class="py-3 px-2 text-center text-slate-200">
                                                <?php echo esc_html($ab['Reserved'] ?? '-'); ?></td>
                                            <td class="py-3 px-2 text-center text-slate-500 font-medium">
                                                <?php echo esc_html($ab['Applied'] ?? '-'); ?></td>
                                            <td class="py-3 px-2 text-right text-primary">
                                                <?php echo esc_html($ab['Times'] ?? '-'); ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
                            ...
                            <table class="w-full text-left">
                                ...
                                <tbody class="divide-y divide-border-navy">
                                    <?php foreach ($details['lot_distribution'] as $lot): ?>
                                        <tr class="text-xs font-bold data-table-row">
                                            <td class="py-3 px-2 text-slate-300">
                                                <?php echo esc_html($lot['Category'] ?? '-'); ?></td>
                                            <td class="py-3 px-2 text-center text-slate-200">
                                                <?php echo esc_html($lot['Lot(s)'] ?? '-'); ?></td>
                                            <td class="py-3 px-2 text-center text-slate-500">
                                                <?php echo esc_html($lot['Qty'] ?? $lot['Shares'] ?? '-'); ?></td>
                                            <td class="py-3 px-2 text-right text-primary">
                                                <?php echo esc_html($lot['Amount'] ?? '-'); ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
                    ...
                    <div class="space-y-2">
                        <?php foreach ($details['documents'] as $doc): ?>
                            <a href="<?php echo esc_url($doc['url']); ?>" target="_blank"
                                class="flex items-center justify-between p-3 rounded-xl bg-slate-900/50 hover:bg-slate-800 border border-white/5 transition-colors group">
                                <span
                                    class="text-[11px] font-bold text-slate-300 group-hover:text-white"><?php echo esc_html($doc['title']); ?></span>
                                <span
                                    class="material-symbols-outlined text-sm text-slate-600 group-hover:text-primary">picture_as_pdf</span>
                            </a>
                        <?php endforeach; ?>
                    </div>
                </div>
