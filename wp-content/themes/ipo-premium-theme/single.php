...
<main class="max-w-[1100px] mx-auto px-4 md:px-6 py-8">
    ...
    while (have_posts()):
        ...
        <nav class="flex items-center text-[10px] md:text-xs uppercase tracking-[0.15em] text-slate-500 mb-8 overflow-x-auto whitespace-nowrap pb-2" aria-label="Breadcrumb">
            <a href="<?php echo home_url(); ?>" class="hover:text-primary transition-colors">Home</a>
            <span class="mx-3 opacity-30">/</span>
            <?php if ($cat): ?>
                <a href="<?php echo get_category_link($cat->term_id); ?>" class="hover:text-primary transition-colors"><?php echo esc_html($cat->name); ?></a>
                <span class="mx-3 opacity-30">/</span>
            <?php endif; ?>
        ...
        <article id="post-<?php the_ID(); ?>" <?php post_class(); ?> itemscope itemtype="https://schema.org/BlogPosting">
            ...
            <header class="mb-10">
                <div class="flex items-center gap-3 mb-6">
                    <span class="bg-[#0d7ff2]/10 text-[#0d7ff2] text-[10px] font-black px-3 py-1 rounded-full border border-[#0d7ff2]/20 uppercase">
                        Live Updates
                    </span>
                ...
                <h1 class="text-3xl md:text-6xl font-black text-white leading-[1.15] mb-8 tracking-tight" itemprop="headline">
                    <?php the_title(); ?>
                </h1>

                <?php if($show_share): ?>
                <div class="flex flex-wrap items-center justify-between gap-6 p-6 rounded-2xl bg-slate-900/40 border border-white/5">
                    <div class="flex items-center gap-4">
                        <div class="w-10 h-10 rounded-full bg-[#0d7ff2] flex items-center justify-center text-white shadow-lg shadow-[#0d7ff2]/20">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        ?>
                        <a href="https://wa.me/?text=<?php echo $title . ' - ' . $url; ?>" target="_blank" class="w-10 h-10 rounded-xl bg-[#25D366]/10 text-[#25D366] flex items-center justify-center hover:bg-[#25D366] hover:text-white transition-all border border-[#25D366]/20">
                           <span class="material-symbols-outlined">share</span>
                        </a>
                        <button onclick="window.print()" class="h-10 px-4 rounded-xl bg-slate-800 text-slate-300 text-xs font-bold border border-white/10 hover:bg-white hover:text-black transition-all">
                            PDF / PRINT
                        </button>
                    </div>
                </div>
            ...
            <div class="flex flex-col lg:flex-row gap-12">
                <div class="lg:w-[70%] order-2 lg:order-1">
                    ...
                    <div class="bg-gradient-to-br from-slate-900 to-[#0d7ff2]/5 border-l-4 border-[#0d7ff2] p-8 rounded-r-2xl mb-12 relative overflow-hidden">
                    ...
                    <div class="entry-content prose prose-invert prose-lg max-w-none prose-headings:text-white prose-p:text-slate-400 prose-a:text-[#0d7ff2] prose-strong:text-white" itemprop="articleBody">
                        <?php the_content(); ?>
                    </div>
                    ...
                    <footer class="mt-16 pt-8 border-t border-white/5">
                        <div class="bg-slate-900/50 rounded-2xl p-6 text-center border border-white/5">
                            ...
                            <p class="text-[10px] text-slate-600 leading-relaxed">
                                The Grey Market Premium (GMP) data provided here is for information purposes only. GMP is volatile and does not guarantee the listing price. Always consult a certified financial advisor before investing in IPOs.
                            </p>
                        </div>
                    </footer>
                </div>
                ...
                <aside class="lg:w-[30%] order-1 lg:order-2">
                    <div class="sticky top-28 space-y-8">
                        <?php if($show_toc): ?>
                        <div id="table-of-contents" class="hidden p-6 rounded-2xl bg-slate-900/80 backdrop-blur-md border border-white/5 shadow-xl">
                            <h4 class="text-white text-xs font-black uppercase tracking-[0.2em] mb-6 flex items-center justify-between">
                                Contents
                        ...
                        <div class="p-6 rounded-2xl bg-[#0d7ff2] text-white">
                            ...
                            <a href="#" class="block text-center py-3 bg-white text-[#0d7ff2] rounded-xl font-bold text-xs uppercase tracking-wider hover:shadow-lg transition-all">Join Telegram</a>
                        </div>
                    </div>
                </aside>
            </div>
            ...
            <section class="mt-20 pt-12 border-t border-white/5">
                <h3 class="text-2xl font-black text-white mb-8 italic">RECOMMENDED UPDATES</h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    if ($related->have_posts()): while ($related->have_posts()): $related->the_post(); ?>
                        <a href="<?php the_permalink(); ?>" class="group">
                            <div class="aspect-video rounded-2xl overflow-hidden bg-slate-900 border border-white/5 mb-4 group-hover:border-[#0d7ff2]/50 transition-all">
                                <?php if (has_post_thumbnail()): the_post_thumbnail('medium_large', ['class' => 'w-full h-full object-cover group-hover:scale-110 transition-transform duration-700']); endif; ?>
                            </div>
                            <h4 class="text-white font-bold leading-tight group-hover:text-[#0d7ff2] transition-colors"><?php the_title(); ?></h4>
                            <p class="text-[10px] text-slate-500 mt-2 font-bold uppercase tracking-widest"><?php echo get_the_date(); ?></p>
                        </a>
